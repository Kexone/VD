...
Vector3 Cell::grad_gramma(Ray &ray, const float t) const
{
	const Vector3 i = u(ray, t);
	const float d = 0.001f;

	float xi = gamma(i - Vector3(d, 0.0f, 0.0f));
	float xn = gamma(i - Vector3(-d, 0.0f, 0.0f));
	float yi = gamma(i - Vector3(0.0f, d, 0.0f));
	float yn = gamma(i - Vector3(0.0f, -d, 0.0f));
	float zi = gamma(i - Vector3(0.0f, 0.0f, d));
	float zn = gamma(i - Vector3(0.0f, 0.0f, -d));

	Vector3 n = Vector3(xi - xn, yi - yn, zi - zn) / (2.0f * d);
	n.Normalize();
...
float Cell::find_iso_surface(Ray &ray, const float t0, const float t1, const float iso_value)
{
	const COMP_TYPE dt = 0.01f;
	float t = t0;
	bool hit = false;
	while (t < t1)
	{
