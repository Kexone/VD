...
public:
	...
	float gamma(Vector3 & ray) const;
	Vector3 grad_gramma(Ray &ray, const float t) const;
	float find_iso_surface(Ray &ray, const float t0, const float t1, const float iso_value);
